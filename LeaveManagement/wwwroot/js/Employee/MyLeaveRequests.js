                ...
                <tr>
                    ...
                    <td>
                        ...
                        ${leave.status === "Pending" ? `
                            <button class="btn btn-sm btn-danger" onclick="cancelLeave(${leave.leaveId})">Cancel</button>
                        ` : ""}
                    </td>
                </tr>
            `).join("")}
...
async function cancelLeave(leaveId) {
    ...
    try {
        const response = await fetch(`${apiBase}/Cancel/${leaveId}`, {
            method: "PUT",
            headers: { "Authorization": `Bearer ${token}` }
        });
        ...
        if (response.ok) {
            ...
            showAlert("danger", result.message || "Failed to cancel leave.");
        }
    } catch (error) {
        console.error(error);
        showAlert("danger", "Unexpected error while cancelling leave.");
    }
}
...
function getStatusBadge(status) {
    ...
    else if (status === "Cancelled") colorClass = "secondary";
